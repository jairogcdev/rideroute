...
router.patch("/coordinates/searchOrigin", async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json(responseAdress.data);
  } catch (err) {
    next(err);
  }
});
...
router.patch("/all", async (req, res, next) => {
  ...
  try {
    const response = await MotoRoute.find().populate({
      path: "user",
      select: [
        "username",
        "motoMake",
...
//GET "/api/routes/search" => search by username in DB

router.patch("/search", async (req, res, next) => {
  const { value } = req.body;
  try {
    const userSearch = await User.find({ username: value }).select({
      username: true,
    });
    const data = await MotoRoute.find({ user: userSearch }).populate({
      path: "user",
      select: [
        "username",
        "motoMake",
